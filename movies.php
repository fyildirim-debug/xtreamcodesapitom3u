...
if (isset($_POST['islemyap'])) {
    ...
    $json = curlRequest($_POST['url'].'/player_api.php?username='.$_POST['username'].'&password='.$_POST['password'].'&action=get_vod_streams');
    $livechannelraw = json_decode($json, true);

    $json = curlRequest($_POST['url'].'/player_api.php?username='.$_POST['username'].'&password='.$_POST['password'].'&action=get_vod_categories');
    ...
    $fullurl = ($url . '/movie/' . $username . '/' . $password . '/');
